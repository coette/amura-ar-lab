...
async function imageLandmarker(){
  ...
  landmarkerPromise=(async()=>{
    ...
    catch(e){console.warn("R13 IMAGE GPU no disponible; CPU",e);landmarker=await HandLandmarker.createFromOptions(files,opts("CPU"));}
    return landmarker;
...
export async function analyzeBankImage(image,item,tokenIsCurrent=()=>true){
  ...
  try{const lm=await imageLandmarker();if(!tokenIsCurrent())return null;mp=lm.detect(canvas);const wrist=mp?.landmarks?.[0]?.[0];if(wrist){p0={x:wrist.x*WIDTH,y:wrist.y*height};p0Source="MediaPipe IMAGE";}}catch(e){mpError=e;console.error("R13 IMAGE",e);}
  ...
  if(!geometry)throw new Error("No se ha podido inicializar el antebrazo");const model=learnModel(clean,geometry);if(!model)throw new Error("No se ha podido aprender la piel");const s=segment(clean,geometry,model,p0),pca=cloudPca(s.mask,WIDTH,height,geometry),final=pca.geometry?finalAxis(s.mask,WIDTH,height,pca.geometry):{centers:[],metric:null};
  return{width:WIDTH,height,clean,mp,mpError,p0,p0Source,geometry,mask:s.mask,pca,final};
}
