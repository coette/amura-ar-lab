import { analyzeBankImage, analysisPoint, directionAngle } from "./static-bank-analyzer.js?v=r13.1";
...
function setBusy(v){document.getElementById("r13BankPrev")?.toggleAttribute("disabled",v);document.getElementById("r13BankNext")?.toggleAttribute("disabled",v);document.getElementById("r13CaptureToggle")?.toggleAttribute("disabled",v||!current);document.getElementById("r13CaptureUpdate")?.toggleAttribute("disabled",v||!current);}
...
async function loadPhoto(file){const r=await fetch(`${BANK_BASE}${file}?v=r13.4`,{cache:"no-store"});if(!r.ok)throw new Error(`foto HTTP ${r.status}`);let bytes=new Uint8Array(await r.arrayBuffer());for(let depth=0;depth<4&&!jpeg(bytes);depth++){const text=new TextDecoder().decode(bytes).replace(/\s+/g,"");if(!text||!/^[A-Za-z0-9+/=]+$/.test(text))break;bytes=b64bytes(text);}if(!jpeg(bytes))throw new Error("la foto guardada no contiene un JPEG válido");const url=URL.createObjectURL(new Blob([bytes],{type:"image/jpeg"}));try{const image=new Image();image.decoding="async";await new Promise((ok,no)=>{image.onload=ok;image.onerror=()=>no(new Error("JPEG no decodificable"));image.src=url;});return image;}finally{setTimeout(()=>URL.revokeObjectURL(url),1000);}}
...
function render(){
  ...
  const values=document.getElementById("r13BankValues");if(values)values.innerHTML=`REPROCESADO · ROI ${fmt(directionAngle(result.geometry?.elbow))} · PCA ${fmt(result.pca.angle)} · FINAL ${fmt(result.final.metric?.angle)}<br>LIVE R12 · ROI ${item.live.roi.toFixed(1)}° · PCA ${item.live.pca.toFixed(1)}° · FINAL ${item.live.final.toFixed(1)}°`;
  const note=document.getElementById("r13BankNote");if(note)note.textContent=`MP IMAGE: ${result.mp?.landmarks?.length?"SÍ":"NO"} · P0: ${result.p0Source} · NUBE: ${result.pca.pixelCount} puntos${item.accumulated?" · 60° en vivo tenía estado acumulado":""}`;
  setStatus("MORADO MEDIAPIPE · AMARILLO ROI/P0 · CYAN MÁSCARA/PCA · BLANCO FINAL");
  updateCaptureUi();
}

function captureFilename(item){return `AMURA_R13_${String(item.target).padStart(3,"0")}deg.png`;}
...
async function makeCapture(){
  ...
  const source=document.getElementById("r13BankCanvas");
  const item=current.item;
  const scale=2, panel=190, out=document.createElement("canvas");
  ...
  const values=document.getElementById("r13BankValues")?.innerText||"";
  const note=document.getElementById("r13BankNote")?.innerText||"";
  ctx.fillStyle="#fff";ctx.font="700 28px Arial";ctx.fillText(`AMURA · R13.2 · POSTURA ${item.target}°`,24,40);
  ctx.font="700 20px Arial";let y=78;
  for(const line of values.split(/\n+/)){ctx.fillText(line,24,y);y+=30;}
  ctx.font="600 17px Arial";ctx.fillStyle="rgba(255,255,255,.82)";ctx.fillText(note.slice(0,95),24,y+8);
  const blob=await canvasToBlob(out);
  return {blob,fileName:captureFilename(item),target:item.target,capturedAt:Date.now()};
}
...
async function exportCaptures(){
  const ordered=[...selectedCaptures.entries()].sort((a,b)=>a[0]-b[0]).map(([,c])=>c);
  if(!ordered.length)return;
  const files=ordered.map(c=>new File([c.blob],c.fileName,{type:"image/png",lastModified:c.capturedAt}));
  try{
    if(navigator.share&&(!navigator.canShare||navigator.canShare({files}))){await navigator.share({files,title:"AMURA · capturas R13.2",text:`${files.length} capturas seleccionadas`});setStatus(`${files.length} CAPTURAS EXPORTADAS`);return;}
...
function ensureUi(){
  ...
  if(!document.getElementById("r13BankStyle")){const s=document.createElement("style");s.id="r13BankStyle";s.textContent=`#r13BankRoot{position:absolute;inset:0;z-index:200000;background:#000;color:#fff;font-family:Arial,sans-serif}#r13BankRoot[hidden]{display:none!important}#r13BankStage{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:205px 0 150px;box-sizing:border-box}#r13BankCanvas{display:block;max-width:100%;max-height:100%;width:auto;height:auto}#r13BankHud{position:absolute;top:calc(env(safe-area-inset-top,0px) + 10px);left:10px;right:10px;z-index:4;padding:10px 12px;border-radius:10px;background:rgba(4,8,14,.87);backdrop-filter:blur(8px);font:800 12px/1.45 Arial,sans-serif}#r13BankTitle{font-size:12px;letter-spacing:.08em;opacity:.76}#r13BankPose{margin-top:2px;font-size:24px}#r13BankValues{margin-top:5px;font-size:13px}#r13BankNote,#r13BankStatus,#r13CaptureCounter{margin-top:5px;opacity:.8}#r13CaptureBar{position:absolute;left:10px;right:10px;bottom:calc(env(safe-area-inset-bottom,0px) + 80px);z-index:6;display:grid;grid-template-columns:1.35fr 1fr 1.35fr;gap:8px}#r13CaptureBar button,#r13BankControls button,#r13BankClose{min-height:48px;border-radius:999px;border:1px solid rgba(255,255,255,.35);background:rgba(5,10,17,.92);color:#fff;font:800 11px Arial,sans-serif;letter-spacing:.03em}#r13CaptureToggle.saved{background:rgba(0,126,89,.92)}#r13CaptureUpdate[hidden]{display:none!important}#r13CaptureBar button:disabled,#r13BankControls button:disabled{opacity:.42}#r13BankControls{position:absolute;left:10px;right:10px;bottom:calc(env(safe-area-inset-bottom,0px) + 16px);z-index:5;display:grid;grid-template-columns:1fr 1fr;gap:10px}#r13BankClose{position:absolute;right:12px;top:calc(env(safe-area-inset-top,0px) + 158px);z-index:6;min-width:88px;min-height:40px}body[data-amura-mode="bank"] #maskLabHud,body[data-amura-mode="bank"] #maskReadyButton,body[data-amura-mode="bank"] #maskResetButton,body[data-amura-mode="bank"] #maskPhotoButton,body[data-amura-mode="bank"] #r12BankHud,body[data-amura-mode="bank"] #r12BankCaptureButton,body[data-amura-mode="bank"] #r12BankExportButton,body[data-amura-mode="bank"] #trackingCanvas,body[data-amura-mode="bank"] #maskCanvas{display:none!important}`;document.head.appendChild(s);}
  if(!document.getElementById("r13BankRoot")){
    const root=document.createElement("section");root.id="r13BankRoot";root.hidden=true;root.innerHTML=`<div id="r13BankStage"><canvas id="r13BankCanvas"></canvas></div><aside id="r13BankHud"><div id="r13BankTitle">R13.2 · BANCO REAL · CAPTURAS SELECCIONABLES</div><div id="r13BankPose"></div><div id="r13BankValues"></div><div id="r13BankNote"></div><div id="r13BankStatus"></div><div id="r13CaptureCounter">SELECCIONADAS: 0 / 8</div></aside><button id="r13BankClose" type="button">CÁMARA</button><div id="r13CaptureBar"><button id="r13CaptureToggle" type="button">✓ GUARDAR CAPTURA</button><button id="r13CaptureUpdate" type="button" hidden>ACTUALIZAR</button><button id="r13CaptureExport" type="button" disabled>EXPORTAR CAPTURAS (0)</button></div><nav id="r13BankControls"><button id="r13BankPrev" type="button">← ANTERIOR</button><button id="r13BankNext" type="button">SIGUIENTE →</button></nav>`;
...
async function showBank(index){bankIndex=(index+BANK.length)%BANK.length;const item=BANK[bankIndex];resetAllState();const token=runToken;setBusy(true);const pose=document.getElementById("r13BankPose"),values=document.getElementById("r13BankValues"),note=document.getElementById("r13BankNote");if(pose)pose.textContent=`POSTURA ${bankIndex+1}/8 · ${item.target}°`;if(values)values.textContent="CARGANDO FOTO…";if(note)note.textContent="resetAllState() antes de cada imagen · sin continuidad temporal";try{const image=await loadPhoto(item.file);if(token!==runToken)return;if(values)values.textContent="MEDIAPIPE IMAGE → MÁSCARA → PCA → FINAL…";const result=await analyzeBankImage(image,item,()=>token===runToken);if(!result||token!==runToken)return;current={item,result};render();}catch(e){console.error("R13 banco",e);if(token===runToken){if(values)values.textContent="ERROR AL REPROCESAR";setStatus(e.message);}}finally{if(token===runToken){setBusy(false);updateCaptureUi();}}}
function openBank(){ensureUi();bankOpen=true;document.body.dataset.amuraMode="bank";document.getElementById("startPanel")?.setAttribute("hidden","");document.getElementById("r13BankRoot").hidden=false;document.title="AMURA · BANCO REAL · R13.2";showBank(bankIndex);}
function closeBank(){resetAllState();bankOpen=false;delete document.body.dataset.amuraMode;document.getElementById("r13BankRoot").hidden=true;const start=document.getElementById("startPanel");if(document.body.dataset.status==="idle"&&start)start.hidden=false;document.title="AMURA · CÁMARA / BANCO · R13.2";}
function boot(){ensureUi();const e=document.querySelector("#startPanel .eyebrow"),t=document.getElementById("cameraTitle"),l=document.getElementById("statusMessage"),p=document.querySelector("#startPanel .privacy");if(e)e.textContent="LABORATORIO · R13.2";if(t)t.textContent="CÁMARA O BANCO DE FOTOS";if(l)l.textContent="Banco: recorre, marca las capturas que quieras y expórtalas juntas al final.";if(p)p.textContent="Puedes quitar, volver a marcar o ACTUALIZAR una captura todas las veces que quieras.";document.title="AMURA · CÁMARA / BANCO · R13.2";}
