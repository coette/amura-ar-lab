// AMURA AR · R17 AUTORRESCATE
// Capa de vigilancia sobre R16. NO cambia verde -> naranja.
...
let lastGoodR16 = null;
let lastGoodR09 = null;
...
function ensureBadge() {
  let badge = document.getElementById("r17RescueBadge");
  if (badge) return badge;
  badge = document.createElement("div");
  badge.id = "r17RescueBadge";
...
function ensureFreezeCanvas() {
  let c = document.getElementById("r17FreezeCanvas");
  if (c) return c;
  c = document.createElement("canvas");
  c.id = "r17FreezeCanvas";
...
function plausibility(snapshot, now) {
  ...
  const fresh = r16 && Number.isFinite(r16.updatedAt) && now - r16.updatedAt < 650 && r16.final;
  if (!fresh) {
    reasons.push("eje");
  } else {
    const centers = r16.baseCenters || [];
...
function saveGood(snapshot) {
  ...
  lastGoodR16 = clone(window.AmuraR16AxisMetrics);
  lastGoodR09 = clone(window.AmuraR09AxisMetrics);
  captureGoodCanvas();
}
...
async function start() {
  ...
  lab.snapshot = function () {
    ...
    if (frozen && lastGoodSnapshot) {
      return {
        ...
        officialAxis: clone(lastGoodSnapshot.officialAxis || lastGoodR09),
        rescueFrozen: true,
  ...
  lab.__r17AutoRescueInstalled = true;

  window.AmuraR17Rescue = {
    revision: "R17",
    get state() {
      return {
        frozen,
