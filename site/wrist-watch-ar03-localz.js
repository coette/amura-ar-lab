...
// AR-03 · EJE ANTEBRAZO · CORRECCIÓN LOCAL-Z
// +X = 9→3 = codo→mano
// +Y = 6→12
// +Z = fondo→cristal
//
// MediaPipe gobierna SIEMPRE el movimiento vivo.
// El borde del antebrazo sólo estima una corrección angular δ alrededor del
// Z vivo. Si el borde falla, δ queda congelada; nunca sustituimos X por la
// línea azul fotograma a fotograma.
...
let lastGateStatus = "esperando borde fiable";
...
function blendAngle(a, b, alpha) {
  ...
  return wrapAngle(a + delta * alpha);
}
...
function detectForearmEdge(options, pose, mediaDirection) {
  ...
  const candidateA = scanOneSide(
    imageData.data, scanWidth, scanHeight, p0, searchDirection, wristWidth, 1
  );
...
function updateCorrection(edge, pose) {
  if (!edge || !edge.detected || edge.held || !edge.direction) {
    if (edge && !edge.held) {
      correctionLocked = false;
      requiredGateFrames = CONFIRM_FRAMES;
      correctionGate.miss();
      lastGateStatus = acceptedCorrectionRad === null
        ? "esperando borde fiable"
        : "borde perdido · δ congelada";
    }
    return acceptedCorrectionRad;
  }

  const target = solveLocalZCorrection(pose, edge.direction);
  if (!Number.isFinite(target)) return acceptedCorrectionRad;
...
export function updateWristWatch(options) {
  ...
  const correctionRad = updateCorrection(edge, pose);
  ...
  if (window.AmuraTrackingDiagnostics) {
    window.AmuraTrackingDiagnostics["Borde antebrazo"] = edge.detected
      ? "detectado · sólo calibra"
      : (edge.held ? "pérdida breve · δ retenida" : "sin borde · δ retenida");
  ...
  return state
    ? { ...state, units: "AR-03 · MediaPipe + δ local-Z retenida" }
    : state;
}
